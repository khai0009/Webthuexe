...
namespace Webthuexe.Areas.Admin.Controllers
{
    ...
    {      
        ...
        public ActionResult calendar(string tang,string giam,string hientai,string dautien)
        {
            ...
            var gio = hour;
            var ngaydau = time.Date.AddDays(-(int)time.DayOfWeek);
            if (hientai == null || hientai == "1")
            {   
                ngaydau = time.Date.AddDays(-(int)time.DayOfWeek);
            }
            if(dautien != null)
            {
            ...
            show.hOADONTHUEXEs = myworldEntities.HOADONTHUEXEs.Where(n=>n.DADATCOC == true).ToList();
